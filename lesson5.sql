-- создаём базу данных для 5 семинара
...
-- Создание представления, в которое попадет информация о пользователях не старше 20 лет.
...
с максимальным количеством сообщений).
*/

-- Вывод пользователей, количество отправленных сообщений и место в рейтинге.
SELECT 
    tab.username,
    tab.cnt_messages,
    DENSE_RANK() OVER(ORDER BY tab.cnt_messages DESC) AS user_rank
FROM (
    SELECT 
...
/* Задача 3
Выберите все сообщения, отсортируйте сообщения по возрастанию даты 
отправления (created_at) и найдите разницу дат отправления между соседними 
сообщениями, получившегося списка.
*/

-- Вывод отсортированных по возрастанию даты сообщений с разницей дат 
-- отправления между соседними сообщениями.
SELECT 
    created_at,
    TIMESTAMPDIFF(SECOND, LAG(created_at) OVER w, created_at) AS diff_prev_time,    
    TIMESTAMPDIFF(SECOND, created_at, LEAD(created_at) OVER w) AS diff_next_time,
    body
FROM messages
